...
async function notifyDispatchersOfNewTrip(trip, user) {
  try {
    ...
    if (trip.managed_client_id) {
      // Facility booking - get client info from joined managed_clients table
      if (trip.managed_clients) {
        const client = trip.managed_clients;
        clientName = `${client.first_name} ${client.last_name}`;
        clientEmail = client.email || 'No email on file';
      } else {
      ...
      // Get facility email from joined facilities table
      if (trip.facilities && trip.facilities.contact_email) {
        clientEmail = `${clientEmail} (Booked by: ${trip.facilities.contact_email})`;
      }
