...
export const PRICING_CONFIG = {
  ...
  COMPANY_OFFICE: {
    ...
    ADDRESS: "5050 Blazer Pkwy, Dublin, OH 43017",
    LAT: 40.0994,
    LNG: -83.1508
  },
  HOLIDAYS: [
...
export async function getPricingEstimate({
  ...
  try {
    ...
    if (calculateDeadMileageEnabled && pickupAddress && destinationAddress && countyInfo && countyInfo.countiesOut >= 2) {
      ...
      // Check if destination is the office (driver returns to office)
      const officeAddress = PRICING_CONFIG.COMPANY_OFFICE.ADDRESS.toLowerCase();
      const isDestinationOffice = destinationAddress.toLowerCase().includes('5050 blazer') ||
                                   destinationAddress.toLowerCase().includes(officeAddress) ||
                                   destinationAddress.toLowerCase().includes('blazer pkwy');

      if (isRoundTrip) {
        // Round trip: charge dead mileage TWICE (to pickup + from dropoff)
        deadMileage = toPickupDistance * 2;
        console.log('🚗 Round Trip - Dead Mileage (TO + FROM):', {
          toPickup: toPickupDistance,
          fromDropoff: toPickupDistance,
          total: deadMileage
        });
      } else if (isDestinationOffice) {
        // One-way ending at office: charge ONCE (only to pickup)
        deadMileage = toPickupDistance;
        console.log('🚗 One-Way (ending at office) - Dead Mileage (TO only):', deadMileage);
      } else {
        // One-way NOT ending at office: charge TWICE (to pickup + return to office)
        const fromDropoffResult = await calculateDeadMileage(destinationAddress);
        deadMileage = toPickupDistance + fromDropoffResult.distance;
        console.log('🚗 One-Way (not ending at office) - Dead Mileage (TO + FROM):', {
          toPickup: toPickupDistance,
          fromDropoff: fromDropoffResult.distance,
          total: deadMileage
        });
      }
    }
