...
export async function checkFranklinCountyStatus(pickupAddress, destinationAddress) {
  try {
    ...
    // Lancaster, OH - Fairfield County (2 counties out) - triggers dead mileage + county surcharge
    ...
    // Check for Lancaster (Fairfield County - 2 counties out)
    const isPickupLancaster = lancasterPatterns.some(pattern => pickup.includes(pattern));
    const isDestinationLancaster = lancasterPatterns.some(pattern => destination.includes(pattern));

    // Lancaster, OH bug fix: Force non-Franklin status (2 counties out)
    if (isPickupLancaster || isDestinationLancaster) {
      console.log('🚨 LANCASTER BUG FIX APPLIED: Lancaster, OH detected as Fairfield County (2 counties out)');
      return {
        isInFranklinCounty: false,
        countiesOut: 2, // Lancaster is 2+ counties out, triggers $4/mile + $50 surcharge + dead mileage
    ...
    // Delaware, OH - Let Google Geocoding API handle it naturally (will return Delaware County, 1 county out)
    ...
    const getCountyFromComponents = (addressComponents) => {
      ...
      if (isOhio) {
        ...
        if (cityComponent) {
          ...
          // Only cities actually IN Franklin County - removed incorrect ones
          // Removed: powell (Delaware), sunbury (Delaware), pataskala (Licking), pickerington (Fairfield)
          const franklinCountyCities = [
            ...
            'new albany', 'blacklick', 'minerva park', 'lockbourne'
          ];
...
/**
 * Get effective rates - merges custom facility rates with defaults
 * Custom rates override defaults where set, null values fall back to defaults
 */
export function getEffectiveRates(customRates = null) {
  if (!customRates) {
  ...
  return {
    ...PRICING_CONFIG,
    BASE_RATES: {
      REGULAR_PER_LEG: customRates.base_rate_regular ?? PRICING_CONFIG.BASE_RATES.REGULAR_PER_LEG,
      BARIATRIC_PER_LEG: customRates.base_rate_bariatric ?? PRICING_CONFIG.BASE_RATES.BARIATRIC_PER_LEG,
    },
    DISTANCE: {
      FRANKLIN_COUNTY: customRates.per_mile_franklin ?? PRICING_CONFIG.DISTANCE.FRANKLIN_COUNTY,
      OUTSIDE_FRANKLIN: customRates.per_mile_outside ?? PRICING_CONFIG.DISTANCE.OUTSIDE_FRANKLIN,
      DEAD_MILEAGE: customRates.dead_mileage_rate ?? PRICING_CONFIG.DISTANCE.DEAD_MILEAGE,
    },
    PREMIUMS: {
      ...PRICING_CONFIG.PREMIUMS,
      WEEKEND_AFTER_HOURS: customRates.weekend_after_hours_fee ?? PRICING_CONFIG.PREMIUMS.WEEKEND_AFTER_HOURS,
      EMERGENCY: customRates.emergency_fee ?? PRICING_CONFIG.PREMIUMS.EMERGENCY,
      HOLIDAY_SURCHARGE: customRates.holiday_surcharge ?? PRICING_CONFIG.PREMIUMS.HOLIDAY_SURCHARGE,
      COUNTY_SURCHARGE: customRates.county_surcharge ?? PRICING_CONFIG.PREMIUMS.COUNTY_SURCHARGE,
      WHEELCHAIR_RENTAL: customRates.wheelchair_rental ?? PRICING_CONFIG.PREMIUMS.WHEELCHAIR_RENTAL,
    },
    DISCOUNTS: {
      VETERAN: customRates.veteran_discount_percent ? customRates.veteran_discount_percent / 100 : PRICING_CONFIG.DISCOUNTS.VETERAN,
    },
  };
}
 ...
 * Updated: August 20, 2025 - All feedback implemented
 * Updated: January 2026 - Added custom facility rates support
 */
export function calculateTripPrice({
  ...
  customRates = null // Custom facility rates (overrides defaults)
}) {
  ...
  let breakdown = {
    ...
    hasCustomRates: !!customRates,
    // Store actual rates used for display in Trip Details (permanent fix)
    rates: {
      baseRatePerLeg: 0,
      perMileRate: 0,
      deadMileageRate: rates.DISTANCE.DEAD_MILEAGE,
      countySurchargeRate: rates.PREMIUMS.COUNTY_SURCHARGE
    }
  };
  ...
  if (isBariatric) {
    breakdown.basePrice = rates.BASE_RATES.BARIATRIC_PER_LEG;
    breakdown.rates.baseRatePerLeg = rates.BASE_RATES.BARIATRIC_PER_LEG;
    if (isRoundTrip) {
      breakdown.roundTripPrice = rates.BASE_RATES.BARIATRIC_PER_LEG;
    }
  } else {
    breakdown.basePrice = rates.BASE_RATES.REGULAR_PER_LEG;
    breakdown.rates.baseRatePerLeg = rates.BASE_RATES.REGULAR_PER_LEG;
    if (isRoundTrip) {
      breakdown.roundTripPrice = rates.BASE_RATES.REGULAR_PER_LEG;
    }
  }
  ...
  if (distance > 0) {
    ...
    if (isInFranklinCounty) {
      breakdown.distancePrice = effectiveDistance * rates.DISTANCE.FRANKLIN_COUNTY;
      breakdown.perMileRate = rates.DISTANCE.FRANKLIN_COUNTY; // Legacy - keep for backward compatibility
      breakdown.rates.perMileRate = rates.DISTANCE.FRANKLIN_COUNTY;
    } else {
      breakdown.distancePrice = effectiveDistance * rates.DISTANCE.OUTSIDE_FRANKLIN;
      breakdown.perMileRate = rates.DISTANCE.OUTSIDE_FRANKLIN; // Legacy - keep for backward compatibility
      breakdown.rates.perMileRate = rates.DISTANCE.OUTSIDE_FRANKLIN;
    }
  }

  // County surcharge for trips outside Franklin County (2+ counties)
  if (countyInfo && countyInfo.countiesOut >= 2) {
    breakdown.countyPrice = (countyInfo.countiesOut - 1) * rates.PREMIUMS.COUNTY_SURCHARGE;
    breakdown.countySurchargeRate = rates.PREMIUMS.COUNTY_SURCHARGE; // Legacy - keep for backward compatibility
    breakdown.rates.countySurchargeRate = rates.PREMIUMS.COUNTY_SURCHARGE;
  }
  ...
  if (deadMileage > 0 && countyInfo && countyInfo.countiesOut >= 2) {
    breakdown.deadMileagePrice = deadMileage * rates.DISTANCE.DEAD_MILEAGE;
    breakdown.deadMileageRate = rates.DISTANCE.DEAD_MILEAGE; // Legacy - keep for backward compatibility
    breakdown.rates.deadMileageRate = rates.DISTANCE.DEAD_MILEAGE;
  ...
  if (pickupDateTime) {
    ...
    if (isAfterHoursTime || isWeekendTime) {
      breakdown.weekendAfterHoursSurcharge = rates.PREMIUMS.WEEKEND_AFTER_HOURS;
    }
  }

  // Emergency fee
  if (isEmergency) {
    breakdown.emergencyFee = rates.PREMIUMS.EMERGENCY;
  }

  // Holiday surcharge (total trip, not per leg)
  if (holidayInfo && holidayInfo.isHoliday) {
    breakdown.holidaySurcharge = rates.PREMIUMS.HOLIDAY_SURCHARGE;
    breakdown.hasHolidaySurcharge = true;
  }

  // Wheelchair rental fee - charged when facility provides wheelchair
  if (wheelchairType === 'provided') {
    breakdown.wheelchairPrice = rates.PREMIUMS.WHEELCHAIR_RENTAL;
  }
...
export async function getPricingEstimate({
  ...
  try {
    ...
    return {
      ...
      customRates: customRates, // Include custom rates in response for transparency
      summary: {
        ...
        hasCustomRates: pricing.hasCustomRates || false,
 ...
 * Updated: Uses actual rates from pricing data instead of hardcoded values
 */
export function createPricingBreakdown(pricing, countyInfo = null) {
  const items = [];

  // Base fare (legs) - Calculate actual per-leg rate from pricing data
  const isBariatric = pricing.isBariatric || false;
  const totalBaseFare = pricing.basePrice + (pricing.roundTripPrice || 0);
  const hasRoundTrip = pricing.roundTripPrice > 0;
  const totalLegs = hasRoundTrip ? 2 : 1;
  const actualBaseRatePerLeg = totalBaseFare / totalLegs;
  const rateName = isBariatric
    ? `${formatCurrency(actualBaseRatePerLeg)}/leg (Bariatric rate)`
    : `${formatCurrency(actualBaseRatePerLeg)}/leg`;
  ...
  // Distance charge with county information - derive actual per-mile rate
  if (pricing.distancePrice > 0) {
    // Use actual county info if available
    let isInFranklin = countyInfo?.isInFranklinCounty !== false;
    const countyName = isInFranklin ? 'Franklin County' : 'Outside Franklin County';

    // Calculate actual per-mile rate from the pricing data
    // We need to reverse-engineer the distance to get the rate
    const perMileRate = pricing.perMileRate || (isInFranklin ? 3 : 4);

    items.push({
      label: `Distance charge (${formatCurrency(perMileRate)}/mile - ${countyName})`,
      amount: pricing.distancePrice,
      type: 'charge'
    });
  }

  // County surcharge - derive actual per-county rate
  if (pricing.countyPrice > 0) {
    const countiesOut = countyInfo?.countiesOut || Math.ceil(pricing.countyPrice / 50);
    const perCountyRate = countiesOut > 0 ? pricing.countyPrice / countiesOut : 50;
    items.push({
      label: `County surcharge (${countiesOut} counties @ ${formatCurrency(perCountyRate)}/county)`,
      amount: pricing.countyPrice,
      type: 'premium'
    });
  }

  // Dead mileage charge - derive actual rate
  if (pricing.deadMileagePrice > 0) {
    // Use stored dead mileage distance if available, otherwise estimate
    const deadMileageRate = pricing.deadMileageRate || 4;
    const deadMiles = pricing.deadMileagePrice / deadMileageRate;
    items.push({
      label: `Dead mileage (${deadMiles.toFixed(1)} mi @ ${formatCurrency(deadMileageRate)}/mile)`,
      amount: pricing.deadMileagePrice,
      type: 'premium'
    });
  }

  // Holiday surcharge - show actual amount
  if (pricing.holidaySurcharge > 0) {
    items.push({
      label: `Holiday surcharge (+${formatCurrency(pricing.holidaySurcharge)})`,
      amount: pricing.holidaySurcharge,
      type: 'premium'
    });
  }

  // Weekend/After-hours surcharge - show actual amount
  if (pricing.weekendAfterHoursSurcharge > 0) {
    items.push({
      label: `Weekend/After-hours surcharge (+${formatCurrency(pricing.weekendAfterHoursSurcharge)})`,
      amount: pricing.weekendAfterHoursSurcharge,
      type: 'premium'
    });
  }

  // Emergency fee - show actual amount
  if (pricing.emergencyFee > 0) {
    items.push({
      label: `Emergency fee (+${formatCurrency(pricing.emergencyFee)})`,
      amount: pricing.emergencyFee,
      type: 'premium'
    });
  }

  // Wheelchair rental - show actual amount
  if (pricing.wheelchairPrice > 0) {
    items.push({
      label: `Wheelchair rental fee (${formatCurrency(pricing.wheelchairPrice)})`,
      amount: pricing.wheelchairPrice,
      type: 'premium'
    });
  }

  // Veteran discount - derive percentage from amount
  if (pricing.veteranDiscount > 0) {
    const discountPercent = pricing.veteranDiscountPercent || 20;
    items.push({
      label: `Veteran discount (${discountPercent}%)`,
