    ...
    return new Promise((resolve, reject) => {
      ...
      directionsService.route({
        ...
        if (directionsStatus === 'OK' && directionsResult.routes && directionsResult.routes.length > 0) {
          console.log(`Found ${directionsResult.routes.length} routes`);
          ...
          // Always use the first route (Google's recommended/fastest)
          // Google Maps API returns routes sorted by their recommendation
          const recommendedRoute = directionsResult.routes[0];
          const leg = recommendedRoute.legs[0];
          const distanceInMiles = leg.distance.value * 0.000621371;
          const duration = leg.duration.text;
          
          console.log('Using Google recommended route (first route):', {
