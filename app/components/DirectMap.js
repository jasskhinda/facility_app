  ...
  useEffect(() => {
    ...
    directionsService.current.route(request, (result, status) => {
      ...
      if (status === 'OK' && result && result.routes && result.routes.length > 0) {
        console.log(`DirectMap: Found ${result.routes.length} routes`);
        ...
        console.log('DirectMap: Selected fastest route (shortest duration)');
        directionsRenderer.current.setDirections({...result, routes: [fastestRoute]});
