...
export default function RouteMapDisplay({ 
  ...
  const calculateRoute = async () => {
    ...
    try {
      ...
      // No cache hit, calculate route with alternatives to find fastest
      const request = {
        origin: origin,
        destination: destination,
        travelMode: window.google.maps.TravelMode.DRIVING,
      ...
      directionsService.route(request, (result, status) => {
        if (status === 'OK' && result && result.routes && result.routes.length > 0) {
          console.log(`RouteMapDisplay: Found ${result.routes.length} routes`);
          ...
          console.log('RouteMapDisplay: Selected fastest route (shortest duration)');
          directionsRenderer.setDirections({...result, routes: [fastestRoute]});
          
          // Extract route information from fastest route
